  ...
  async function handleEmailLogin(e: React.FormEvent) {
    ...
    if (error) {
      setError(error.message === "Invalid login credentials"
        ? i("auth.login.invalidCredentials")
        : error.message
      );
      setLoading(false);
      return;
    ...
    <div className="min-h-screen flex items-center justify-center bg-surface-tertiary px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-8">
          ...
          <p className="mt-3 text-sm text-text-muted">{i("auth.login.subtitle")}</p>
        </div>
        ...
        <div className="rounded-xl border border-border bg-surface-secondary p-6 shadow-sm">
          ...
          <div className="mt-3 text-center">
            <Link href="/auth/reset-password" className="text-xs text-text-muted hover:text-brand-500 transition">
              {i("auth.login.forgot")}
            </Link>
          </div>
        </div>
